...
public class FilterDialog extends JDialog {
    ...
    private JLabel databasesLbl;
    private JLabel filedsLbl;
    ...
    public FilterDialog() {
        setAlwaysOnTop(true);
        setBounds((Util.getScreenWidth() - DEFAULT_WIDTH) / 2, (Util.getScreenHeight() - DEFAULT_HEIGHT) / 2, DEFAULT_WIDTH, DEFAULT_HEIGHT);
        ...
        final MysqlMetadata metadata = MysqlMetadata.getInstance();

        Object[] databases = metadata.getDatabaseList();
        databaseList.setListData(databases);
        databaseList.addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent e) {
                String database = (String) databaseList.getSelectedValue();
                Object[] tables = new TreeSet(metadata.getTableList(database)).toArray();
        ...
        tableList.addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent e) {
                ...
                if (tableList.getSelectedValue() != null) {
                    ...
                    List<MysqlField> mysqlFields = metadata.getFieldList(database, table);
                    String[] fields = new String[mysqlFields.size()];
                    for(int i = 0; i< mysqlFields.size(); i++){
                    	fields[i] = mysqlFields.get(i).getName();
                    }
        ...
        for (EventFilter ef : Config.getInstance().getIgnoreFilters()) {
            selectedListModel.addElement(ef);
        }
        ...
        ignoreBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (databaseList.getSelectedValue() != null) {
                    database = (String) databaseList.getSelectedValue();
                }
                if (tableList.getSelectedValue() != null) {
                    table = (String) tableList.getSelectedValue();
                }
                if (fieldList.getSelectedValue() != null) {
                    field = (String) fieldList.getSelectedValue();
                }

                EventFilter filter = new EventFilter(database, table, field);

                int index = selectedList.getSelectedIndex();
                selectedListModel.addElement(filter);
        ...
        deleteFilter.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (selectedList.getSelectedIndex() != -1) {
                    selectedListModel.removeElementAt(selectedList.getSelectedIndex());
                }
            }
        });
    ...
    private void onOK() {
        ...
        List<EventFilter> ignoreList = new ArrayList<EventFilter>();
        while (elements.hasMoreElements()) {
            ignoreList.add((EventFilter) elements.nextElement());
        }
        config.setIgnoreFilters(ignoreList);
        config.saveConfig();
